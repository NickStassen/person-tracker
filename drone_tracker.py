...
This script integrates ultrasonic sensors into the drone control application.
Two ultrasonic sensors are used: one facing forward (for detecting objects ahead)
and one facing upward (for detecting the ceiling). Their readings are acquired continuously
in a separate thread. The script also performs person detection/tracking using a computer vision
model, provides a Flask-based web interface for video streaming and flight commands,
and uses DroneKit for vehicle control.
...
def measure_distance(trig_pin: int, echo_pin: int) -> tuple:
    ...
    Returns:
        tuple: Measured distance in meters and feet.
    """
...
def ultrasonic_loop() -> None:
    ...
    while True:
        ...
        print(
            f"Front Sensor: {front_sensor_distance[0]:.2f} m, {front_sensor_distance[1]:.2f} ft"
        )
        print(
            f"Up Sensor: {up_sensor_distance[0]:.2f} m, {up_sensor_distance[1]:.2f} ft"
        )
        time.sleep(2)
...
last_bbox = None
frame_count = 0
MODE = "follow"  # Modes: "gps" or "vision"; here "follow" means tracking is active.
GPS_FOLLOW = True  # Set to True for GPS mode (with orientation corrections), False for vision-only mode.
...
def detect_and_track() -> None:
    ...
    the tracker attempts to follow the person. If the person deviates from the image center,
    the drone’s position is updated. When GPS_FOLLOW is enabled, the system uses the drone’s heading
    to convert image error into a global adjustment.
    """
    ...
    while cap.isOpened():
        try:
            ...
            if not ret:
                continue
            frame_count += 1
            # If not in follow mode or vision-only mode is active, simply update the frame
            if MODE != "follow" or not GPS_FOLLOW:
                with lock:
                    output_frame = frame.copy()
                continue

            # Every DETECTION_INTERVAL frames or if tracker is lost, re-detect
            if frame_count % DETECTION_INTERVAL == 0 or tracker is None:
                ...
                for i in range(detections.shape[2]):
                    ...
                    if (
                        ...
                        and CLASS_NAMES[class_id] == "person"
                    ):
                ...
                if success:
                    ...
                    if abs(dx) > CENTER_TOLERANCE or abs(dy) > CENTER_TOLERANCE:
                        # Convert image error to body-frame error.
                        # With the camera pointed down, top of the image is forward,
                        # so we reverse the vertical error.
                        error_x = dx  # Right is positive.
                        error_y = -dy  # Forward is positive.
                        # Get current heading (in radians) from the drone.
                        theta = math.radians(vehicle.heading)
                        # Rotate error vector from the body frame to the global frame.
                        global_error_lon = error_x * math.cos(
                            theta
                        ) - error_y * math.sin(theta)
                        global_error_lat = error_x * math.sin(
                            theta
                        ) + error_y * math.cos(theta)
                        current_location = vehicle.location.global_relative_frame
                        new_lat = (
                            current_location.lat + ADJUSTMENT_STEP * global_error_lat
                        )
                        new_lon = (
                            current_location.lon + ADJUSTMENT_STEP * global_error_lon
                        )
                        target = LocationGlobalRelative(
                            new_lat, new_lon, current_location.alt
                        )
                        print(
    ...
    while True:
        try:
            ...
            if not ret:
                continue
            with bbox_lock:
                if MODE == "follow" and not GPS_FOLLOW and last_bbox:
                    x, y, w, h = last_bbox
...
def handle_location():
    ...
    if request.is_json:
        ...
        return (
            jsonify(
                {
                    "status": "Command received, flying to new location at 2 meters altitude"
                }
            ),
            200,
        )
